...
// Memoized outside the component so positions are stable across re-renders
const ANIMATION_NODES = Array.from({ length: 11 }, (_, i) => ({
  id: i,
  left: `${(i * 9.1 + 5) % 100}%`,
  top: `${(i * 7.3 + 10) % 100}%`,
...
const Index = () => {
  ...
  // Stable node list — useMemo prevents any accidental re-computation
  const animationNodes = useMemo(() => ANIMATION_NODES, []);
    ...
    <ScrollArea className="min-h-screen">
      <div className={`min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 transition-opacity duration-500 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <main className="relative">
          ...
          {/* Animated Knowledge Graph Nodes — positions are stable (no Math.random in render) */}
            ...
            {animationNodes.map((node) => (
              <div
                ...
                style={{
                  left: node.left,
                  top: node.top,
                  animation: `pulse ${node.duration} ease-in-out infinite`,
